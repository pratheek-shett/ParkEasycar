    ...
    Scaffold(scaffoldState = scaffoldState, floatingActionButton = {
        FloatingActionButton(onClick = { viewModel.onEvent(MapEvent.ToggleFalloutMap) }) {
            Icon(
                ...
                } else Icons.Default.ToggleOn, contentDescription = "Toggle Button New Mode"
            )
        ...
        GoogleMap(
            ...
            onMapClick = {viewModel.onEvent(MapEvent.OnMapLongClick(it))

            }


            ){
            viewModel.state.parkingSpots.forEach{spot -> Marker(
                position = LatLng(
                    spot.lat,
                    spot.lng
                ),
                title = "You Have Parked Here (${spot.lat},${spot.lng})",
                snippet = "Long Press to Remove Slot",
                onInfoWindowLongClick = {viewModel.onEvent(MapEvent.OnInfoWindowLongClick(spot))},
                onClick = {it.showInfoWindow()
                    true},
